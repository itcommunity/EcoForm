...
public class MemberRepo implements IMember {
    ...
    public Member add(Member member) {
        String sql_template = "INSERT INTO Members (NAME,BIRTHDAY,PHONE,EMAIL,FACEBOOK,"
                ...
                + "AREA,MENTOR,ORGANAZATION,MANAGERSEVENTS,HOBBY)"
                + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        String[] values = new String[]{member.getName(), member.getBirthday(), member.getPhone(),
                member.getEmail(), member.getFacebook(), member.getPostaddress(), member.getTimeWithUs(),
                member.getSkills(), member.getResponsability(), member.getHands(), member.getArea(),
                member.getMentor(), member.getOrganazation(), member.getManagersEvents(), member.getHobby()};
